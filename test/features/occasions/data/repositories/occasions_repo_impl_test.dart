...
void main() {
  ...
  OccasionsRepositoryImpl repositoryImpl = OccasionsRepositoryImpl(dataSource, networkConnection);

  test('when call getAllOccasions with no internet should return ApiErrorResult', () async{
  ...
  test('when call getAllOccasions should return ApiSuccessResult when successfully', () async{
    when(networkConnection.isConnected).thenAnswer((_) async => true);

    final expectedResponse = OccasionsResponseDto(products: [
      ProductsDto(
        id: "1"
      ),
      ProductsDto(
